...
export async function assignPermissionToGroup(config, req, res) {
  ...
  const group = await Group.query()
    ...
    .first()
    .catch((err) => { throw [ err, 'assignPermissionToGroup.findGroup' ]; });
  ...
  const permission = await Permission.query()
    ...
    .first()
    .catch((err) => { throw [ err, 'assignPermissionToGroup.findPermission' ]; });
  ...
  return await transaction(Group.knex(), async (trx) => {
    ...
    if (existingPermission) {
      if (res) {
        res.status(400);
        res.json({
          status: 'PERMISSION_ALREADY_ASSIGNED_TO_GROUP',
          message: 'This group already has the specified permission.',
        });
      }
      return false;
    }

    await group.$relatedQuery('permissions', trx).relate(permission.id);
    return true;
  });
}